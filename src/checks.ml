...
module Make (K : Data.Key) (V : Data.Value) (IO : Io.S) = struct
  ...
  module Integrity_check = struct
    ...
    let print_window_around offset io context =
      let window_size = (2 * context) + 1 in
      List.init window_size (fun i ->
          let index = i - context in
          Int64.(add offset (mul (of_int index) encoded_sizeL), index = 0))
      |> List.filter (fun (off, _) -> Int64.compare off 0L >= 0)
      |> List.map (fun (off, highlight) ->
             let entry = IO.read_entry io off in
             (if highlight then Fmt.(styled (`Fg `Red)) else Fun.id)
               (fun ppf () -> (Repr.pp Entry.t) ppf entry))
      |> Fmt.(concat ~sep:cut)
