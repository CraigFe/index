...
module Event = struct
  ...
  let to_json_fields : t -> (string * Json.t) list = function
    ...
    | Duration_end { pid; tid; ts; args } ->
      let fields =
        [ ("tid", `Int tid); ("pid", `Int pid); ("ts", Timestamp.to_json ts); phase "E" ]
      in
  ...
  let duration_start common = Duration_start (common, [], None)

  let duration_end ?args ~pid ~tid ~ts = Duration_end { args; pid; tid; ts }
